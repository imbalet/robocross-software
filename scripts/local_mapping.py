...
def remap_robot_coord(current: Odometry, map_array: np.ndarray, map_resolution: float):
    #определяет глобальные координаты середины карты
...
class LocalMapping(Node):
    ...
    def __init__(self, node_name: str):
        ...
        self.declare_parameter('map_topic', '/local_map')
...
#ищем препятствия и рисуем вокруг них круг
...
#рисование карты
    def set_scan(self, scan_msg:LaserScan, scan_pos):
        if scan_msg != LaserScan():
            ...
            #позиция сканера относительно центра карты
            sensor_x, sensor_y = polar_to_decart(rho, base_yaw)
            #глобальная позиция сканера
